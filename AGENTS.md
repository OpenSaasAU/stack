...
- **AGENTS.md:** This file contains general guidance and architectural patterns
- **README files:** Each package and example has its own README for specific usage instructions
- **Codex Skills:** Specialized skills for common tasks are in `.Codex/skills/`
  - `pr-changeset`: **REQUIRED** - Use when modifying any package code to create proper changeset files
  - `plugin-version`: **REQUIRED** - Use when modifying any Codex plugin code to bump plugin and marketplace versions
...
A `resolveOutput` hook sees **exactly its own declared dependency set plus the list's system fields** — never another computed field's output, and never what the caller happened to select. Reaching for anything it did not declare finds nothing there. See `needs` in `packages/core/AGENTS.md` and ADR-0051.
...
**See:** `packages/auth/AGENTS.md` for detailed patterns and `examples/auth-demo` for usage.
...
**See:** `packages/core/AGENTS.md` and `packages/auth/AGENTS.md` for detailed patterns, and `examples/mcp-demo` for usage.
...
The `session` object passed to access control functions is user-defined. The stack only requires that it exists and does not enforce a structure. The common pattern is an object carrying a user id, or `null` for anonymous. Augment the `Session` interface to type it — see `packages/core/AGENTS.md`.
...
**IMPORTANT:** When working with Codex, you MUST use the `pr-changeset` skill to create changeset files. The changeset CLI doesn't work in the Codex environment, so use the skill instead.
...
**Manual changeset creation (if not using Codex):**
...
Codex plugins in `Codex-plugins/*` use direct semver versioning in JSON files. Whenever you modify plugin code, skills, commands, agents, or marketplace root files, you must bump the version using the `plugin-version` skill.

**IMPORTANT:** When working with Codex, you MUST use the `plugin-version` skill to bump plugin versions. It handles both the plugin's own `plugin.json` and the matching entry in `.Codex-plugin/marketplace.json`.

The `plugin-version` skill:

- Detects which plugin directories changed (`Codex-plugins/opensaas-stack/`, `Codex-plugins/opensaas-migration/`, `.Codex-plugin/`)

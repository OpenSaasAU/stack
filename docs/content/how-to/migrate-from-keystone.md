...
- **Recipe 2 — `connect` / `disconnect` / `set` nested writes.** `data` is passed straight through to Prisma, so use Prisma's relation-operation shapes; `set: []` clears a to-many relation, and you never write the scalar `<field>Id` directly (it is silently stripped). **Note:** nested `set`/`updateMany`/`deleteMany` are refused for non-sudo contexts (`NestedRelationInputError`) — see [Nested `disconnect`/`set`/`updateMany`/`deleteMany` access](/docs/concepts/access-control#nested-disconnect-is-gated-by-target-query-access-setupdatemanydeletemany-are-refused). Rewrite a Keystone `set: []`/`updateMany`/`deleteMany` as writes against the target list directly, wrapped in `context.transaction()` when they must land atomically.

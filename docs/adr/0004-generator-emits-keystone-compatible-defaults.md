...
- **Auto-timestamps are off by default.** _(Rationale amended by ADR-0048: this default survives on greenfield grounds — a generator should not add columns nobody declared — not on the Keystone-migration grounds recorded below.)_ The generator no longer appends `createdAt`/`updatedAt` to every model. A list opts in by declaring the fields itself or via `db: { timestamps: true }`; when timestamps are enabled and the list also declares its own `createdAt`/`updatedAt`, the auto pair is skipped (no duplicate-field `P1012`). This is a breaking change to existing stack apps and is acceptable in beta.
- **Singleton `id` is bare.** _(Reversed by ADR-0048: `Int @id @default(1)` is restored, the Keystone-parity reason having gone out of scope.)_ Singleton lists emit `id Int @id` (no `@default(1)`), matching Keystone 6.

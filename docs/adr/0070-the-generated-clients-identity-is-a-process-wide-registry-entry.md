...
- `@opensaas/stack-core/internal` gains `processGlobal` and `processGlobalKey`. Neither carries a new semver guarantee beyond what the subpath already states — generated code and sibling packages only.
...
- **`packages/auth/tests/generated-project.ts` reached into the OLD key by its bare string name** (`Reflect.get(globalThis, 'opensaasClient')`) to close and drop the cached client between the sequentially-generated projects its "one project at a time per process" harness runs (`rate-limit-e2e.test.ts`'s three tests, each a fresh generated project over its own database). That reach-in silently stopped finding anything once the key moved to `Symbol.for(processGlobalKey('client'))`, so its cleanup became a no-op: the next project's `getClient()` found the previous, already-closed project's client still published, passed `isRuntimeClient`'s structural check, and adopted it — surfacing as `ECONNREFUSED` and "database does not exist" against a torn-down project's connection. Caught by CI on this PR's own head, not found in review. Fixed by updating the harness to compute the same key through the now-exported `processGlobalKey`, in both `generated-project.ts` and the assertion in `rate-limit-e2e.test.ts` that the slot is empty after `close()`.

## Known limits

The registry key (`'client'`, namespaced by `processGlobalKey` to `@opensaas/stack-core/client`) names **the slot**, not a particular app, config, or database — `isRuntimeClient` verifies only that a published value looks like a Prisma runtime client, not that it is _this_ app's. Two structurally-compatible clients from different configs sharing one process at the same time, with neither closing and dropping its own before the other constructs, would have the second silently adopt the first's pool against the wrong database. This repository has exactly two in-process-multiple-client shapes, and both are safe under this design: `bundle-client-construction.test.ts`'s two-copies probe is the _same_ config's own client, which is the point; `generated-project.ts`'s sequence of generated projects is different configs, but strictly serialised with an explicit close-and-drop between them (enforced by convention, not by a lock this module provides). A consumer that needs multiple **different**, live-at-once generated bundles in one process — not something anything in this codebase does today — would need to widen the registry key to include something that identifies the config (a hash of the resolved contract, say), which this record does not do because nothing here exercises that shape.

...
- **A hook reading an undeclared column breaks silently**, exactly as ADR-0025's undeclared relation did. This is the largest cost of the record. Detection leads the changeset: find `resolveOutput` hooks whose `item` reads a key absent from that field's `needs`. _(Narrowed by [ADR-0052](0052-the-generated-types-declare-the-contract-remainder-and-instantiate-core-generics.md): the same generation pass also emits each field's set as a type, so a `resolveOutput` hook typed through `Lists.<List>.TypeInfo` sees `item` as exactly its declared set plus the list's system fields, and an undeclared read is a compile error. The break stays silent only for a hook authored without `TypeInfo`.)_

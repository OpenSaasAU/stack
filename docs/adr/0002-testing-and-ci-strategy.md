...
That re-baseline happened in [#1156](https://github.com/OpenSaasAU/stack/issues/1156), which deleted `packages/core/tests/` and left the whole core corpus colocated under `src/`. The thresholds moved to `src/access/**` 76/71/78/81, `src/context/**` 86/80/92/90 and `src/validation/**` 92/86/98/95 (statements/branches/functions/lines) — every number at or above the one it replaced, each keyed in the config to the file that is currently lowest in its glob. No exclusion was added; test files are already outside the report.

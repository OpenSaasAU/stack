...
**Silent failure**:
The convention that an access-denied operation returns `null` (single) or `[]` (many) rather than throwing, so callers cannot distinguish "denied" from "does not exist". A relation the reading session may not see is `null` (to-one) or `[]` (to-many) in the row it hangs off, and is typed that way whatever its column says — a required to-one is required to write and nullable to read (ADR-0058). A denial is atomic with the result it replaces — a caller never receives part of a read that was refused, which is why a read arrives whole rather than a row at a time (see Terminal operation).
...
**Unsafe surface**:
The deliberately unsecured client, reached under a name that states the bypass. It carries neither access control nor hooks, and every query executed through it — ORM, typed SQL or raw — is stamped as intentionally unscoped, so a bypass is an audited act rather than an absence indistinguishable from a mistake; a query bearing no stamp at all belongs to neither surface and is refused. Better-auth's own flows run here by design (ADR-0013, ADR-0038). It is also the only address for application-authored SQL: it hands out the ORM's typed SQL builder and raw tag untouched and executes their plans through its own executors, which apply the stamp at execution — a plan is an unstamped value until the surface runs it, and no application ever writes a stamp (ADR-0056). Scoping such a query is the caller's alone. It has a transaction-bound shape, reached from the transaction context.

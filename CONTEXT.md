...
The first pass of a read, run before the database is hit. Uses operation-level access to build the access-scoped `include`/`where` so the database only returns rows and relations the session is allowed to see. It scopes the relations a read asked for, and leaves out one the session may not read at all when that can be decided without a row; otherwise it does not choose which relations a read fetches (see Bare read). Leaving one out is a saving, never the decision — Field Visibility checks every relation it is handed regardless (ADR-0044). Failing to compute a scope is a **denial**, never a passthrough: a caller-supplied `include` nested deeper than the phase can scope throws rather than returning unscoped rows (ADR-0022), and the same rule covers a key the list config never declared a relationship for — an unrecognised or synthetic key named in a caller's `include` is denied, not passed through, exactly as a too-deep one is (ADR-0061). This is distinct from having nothing to scope — a list with no relationships — which passes through unchanged. Not every read reaches this phase's include construction today: a fragment read (`{ query: fragment }`) bypasses it entirely rather than being denied by it — a tracked gap, not a second rule (#1088).
_Avoid_: query builder, include builder

**Synthetic back-relation**:
The key schema generation adds to the target of a list-only `ref` (`from_<List>_<field>`) because the ORM requires an opposite field there, even though no list config declares one. It is reachable in `include` because the generated `Include` type is built additively on the ORM's own, so a caller can name it — and wherever a caller can name it, it is treated as the declared relationship field it stands for, always to-many, so the Access Filter and Field Visibility scope, gate and compute through it exactly as they would a declared relationship (the write path resolved it first, #978).
_Avoid_: reverse relation, back-reference, implicit relation
...
**Engine stamp**:
A mark the access engine puts on every query it executes, and the only thing the ORM-level tripwire reads. It is ambient rather than attached: the surface executing a query enters an async-context origin around exactly that execution, so every statement the ORM issues for one logical call — the identity lookups and related-row writes a per-plan mark cannot reach — carries it (ADR-0059). It carries no session and no policy — it answers one question, "did this execution come through a declared surface?", which is what lets a query neither surface executed be refused before it compiles (ADR-0038). Because it says nothing about _who_ is reading, scoping a query to a session stays an ordinary rebind. The mark is entered inside the terminal around the ORM call it issues, never around a hook and never at an application call site, and the tripwire that reads it is stack-owned rather than configured — the two are one component that is removed together or not at all (ADR-0049).
_Avoid_: query tag, session token, middleware context, annotation (its first mechanism; the term names the guarantee, not how it is carried)

**Unsafe surface**:
The deliberately unsecured client, reached as `context.unsafe` — a name that states the bypass. It carries neither access control nor hooks, and every query executed through it — ORM, typed SQL or raw — is marked as intentionally unscoped, so a bypass is an audited act rather than an absence indistinguishable from a mistake; a query bearing no mark at all belongs to neither surface and is refused. Better-auth's own flows run here by design (ADR-0013, ADR-0038). It is also the only address for application-authored SQL: it hands out the ORM's typed SQL builder and raw tag untouched and executes their plans through its own executors, and its ORM lane is a transparent proxy over the ORM's own collections; each enters the unsafe origin around the execution it performs, so a plan or a query value is unmarked until the surface executes it and no application ever writes a mark (ADR-0056, ADR-0059). It exposes neither the bare client nor its `prepare()`, since a prepared statement executes unobserved by the tripwire. Scoping such a query is the caller's alone. It has a transaction-bound shape, reached from the transaction context.
_Avoid_: raw client, escape hatch, prisma passthrough, context.sql

**ORM handle**:
The engine's own ORM client, carried as `ormHandle` on the `AccessContext` and narrowed to one model by `ormModel()`. It is what the secured surface's terminals, the Write Pipeline and the access filter actually issue their queries through: the engine applies the Access Filter, Field Visibility and hooks _around_ it, so the handle itself enforces none of them. The Write Pipeline rebinds it to the write's transaction alongside `context.db`, which is why database work a hook does through it rolls back with the write. It is not the Unsafe surface — that is `context.unsafe` on the request context, the application's documented bypass, and it is not a member of `AccessContext`. Plugin hooks and `runtime()` factories are handed an `AccessContext`, so the handle is what a plugin author reaches, and it bypasses as much as `unsafe` does.
_Avoid_: `context.prisma` (its name before #1144's rename landed), raw client, internal prisma

**Extension pack**:
A bundle of database capability — column types, codecs, typed operations — contributed by a package and named in `db.extensions` as an import descriptor (`{ name, from }`, `from` being the package name) rather than as a value, so the generated contract, config and runtime can each import the façade they need. Declaring one makes `Vector(n)` a column type the schema can use and brings the pack's Extension contract space into the project, through which Prisma itself installs the Postgres extension; the deployment's job is to make the extension available and the privilege to create it (ADR-0065). A pack may be declared by the application or contributed by a plugin that knows it needs one; a field whose type names a pack the config does not declare fails generation (ADR-0049).
_Avoid_: preview feature, database extension, plugin, adapter

**Extension contract space**:
The migration space an Extension pack ships with itself — a fixed migration package that enables the Postgres extension, a head ref and a contract snapshot — which `pnpm generate` materialises under `migrations/` as the fourth emission of a `db.extensions` entry, committed and regenerated like the emitted contract. Prisma's reconcile and migrate paths both refuse a declared pack whose space is not on disk, and both plan its `CREATE EXTENSION` ahead of the app's DDL, skipping it when the extension already exists (ADR-0065).
_Avoid_: extension migration, install migration, migrations/pgvector, seed phase
...
**Row lock**:
A `SELECT … FOR UPDATE` taken through the secured surface, reachable only on a transaction-bound builder because outside a transaction it is a silent no-op. It is the stack's concurrency gate now that no isolation level can be selected (ADR-0042): the contended parent row is locked as a **mutex token** — every racer takes the same token before reading the count it will branch on — rather than as protection for that row's own columns. The engine issues it as a second statement over the primary keys the scoped read returned, always ordered by primary key, so the locked set is provably a subset of the readable set and acquisition order is the same in every session (ADR-0047). A terminal never returns a row it did not lock. The ORM has no locking clause, so the terminal composes that statement itself through the ORM's raw lane — names quoted and keys bound by Prisma's own helpers with the identity column's codec, run by the transaction's executor — and the stack authors only the locking clause (ADR-0062).
_Avoid_: pessimistic lock, select for update, transaction lock, raw lock (it is raw only in how the engine lowers it)
...
A `beforeCompile` registered beside the engine's stamp in a test, capturing each compiled plan so a test can assert on the query the engine built — the merged Access Filter, the widened selection, the Engine stamp, which it reads from the same ambient origin the tripwire reads — without any seam on the secured surface. It observes and passes through; it never rewrites and never skips the tripwire.

...
a set of files and the filesystem offers no multi-file commit, so `prisma/contract.json`
— the file the app reads — lands first and the project-root `prisma.config.ts` lands
last. Between those two the test could append its "held back until promotion" stamp to a
root config the loop was about to overwrite, and the next assertion then read a
regenerated file with no stamp in it.

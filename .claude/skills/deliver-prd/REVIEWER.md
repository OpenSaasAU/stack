...
Dispatch as a **background** general-purpose agent. The reviewer gets the PR and **nothing else** — no PRD, no implementer notes, no design context from this conversation. That independence is the whole point: a reviewer primed with the author's intent rubber-stamps it.

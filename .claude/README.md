...
The Claude Code **plugin** (commands, agents, skills) is located in:

```
.claude-plugin/
```

That's where the `plugin.json`, commands, agents, and skills live.

## Why Two Directories?

- **`.claude/`** - Repository-level settings (like auto-connecting MCP servers)
- **`.claude-plugin/`** - Distributable plugin with commands/agents/skills

When you trust the OpenSaaS Stack repository folder:

1. Settings from `.claude/settings.json` are applied (MCP server connects)
2. Plugin from `.claude-plugin/` is loaded (commands/agents/skills available)

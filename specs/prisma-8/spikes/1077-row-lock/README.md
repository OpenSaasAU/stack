...
| B11  | empty key set: `WHERE "id" IN ()`                                           |                                                                                             | **throws** `syntax error at or near ")"`                                                       |
| B12  | 65,600 bound keys                                                           |                                                                                             | **throws** `bind message has 65 parameter formats but 0 parameters` — the 16-bit count wrapped |
| B12b | 60,000 bound keys                                                           |                                                                                             | executes (0 rows — keys are synthetic)                                                         |

B11 and B12 were added on review: an empty scoped read must return the terminal's empty value without issuing the lock, and a multi-row lock must be bounded well under the 65,535-parameter ceiling — over it, the failure is a corrupted bind rather than a clean refusal.

...
const q = db.orm.public.Post.where({ authorId: 'u1' })
  .where((p: any) => p.published.eq(true))
  .orderBy((p: any) => p.title.asc?.() ?? p.title)
  .limit(5)
console.log(
  'Collection class name:',
  q.constructor.name,
  ' instanceof exported Collection:',
  q instanceof (ormClient as any).Collection,
)
...
console.log(
  'filters:',
  st.filters.length,
  st.filters.map((f: any) => f.constructor.name),
)
...
console.log(
  'immutability: base state filters =',
  db.orm.public.Post.state.filters.length,
  ' derived =',
  q.state.filters.length,
)

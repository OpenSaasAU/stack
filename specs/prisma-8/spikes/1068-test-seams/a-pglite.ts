...
const seed = async (pool: pg.Pool) => {
  ...
  await pool.query(
    `insert into "Post" values ('p1','a1',true,'u1'),('p2','a2',false,'u1'),('p3','b1',true,'u2') on conflict do nothing`,
  )
}
...
for (const mode of ['unix', 'tcp'] as const) {
  for (let i = 0; i < 3; i++) {
    ...
    console.log(
      `  ${mode} run ${i + 1}: pglite=${s.ms.pglite.toFixed(0)}ms socket=${s.ms.socket.toFixed(1)}ms total=${s.ms.total.toFixed(0)}ms`,
    )
    await s.stop()
  }
}
{
  ...
  const s = await startPglite({ mode: 'unix', withVector: false })
  console.log(`  unix (no pgvector ext): total=${s.ms.total.toFixed(0)}ms`)
  await s.stop()
}
...
console.log('\n== unix socket: postgres({ url }) vs pg: new Pool({ host: dir }) ==')
{
  ...
  try {
    ...
    console.log('  url=?host=<dir> FAILED:', e.message)
  }
  ...
  try {
    const db: any = postgres({
      contract,
      url: `postgres://postgres@${encodeURIComponent(dir)}/postgres`,
    })
    console.log(
      '  url with encoded dir as host ->',
      (await db.orm.public.Post.all()).length,
      'rows OK',
    )
    await db.close()
  } catch (e: any) {
    console.log('  url with encoded dir as host FAILED:', String(e.message).slice(0, 120))
  }
  // (c) pg: Pool({ host: dir })
  try {
    const pool = new pg.Pool({ host: dir, database: 'postgres', user: 'postgres', max: 1 })
    ...
    console.log('  pg: Pool({host: dir}) FAILED:', e.message)
  }
  ...
  try {
    ...
    console.log('  pg: Client({host: dir}) FAILED:', e.message)
  }
  await s.stop()
}
...
for (const mode of ['unix', 'tcp'] as const) {
  ...
  console.log(
    `  ${mode}: orm .where().all() = ${ms.toFixed(3)} ms/query   raw pg.query = ${ms2.toFixed(3)} ms/query`,
  )
  await db.close()
  await pool.end()
  await s.stop()
...
{
  const a = await startPglite({ mode: 'unix' })
  const b = await startPglite({ mode: 'unix' })
  const pa = new pg.Pool({ connectionString: a.url, max: 1 })
  const pb = new pg.Pool({ connectionString: b.url, max: 1 })
  await seed(pa)
  await pb.query(DDL)
  await pb.query(`insert into "User" values ('zz','zed')`)
  const da: any = postgres({ contract, pg: pa })
  const dbb: any = postgres({ contract, pg: pb })
...
for (const maxConnections of [1, 5]) {
  ...
  const outcome = await Promise.race([
    Promise.all(
      Array.from({ length: 5 }, (_, i) =>
        db.orm.public.Post.where({ authorId: i % 2 ? 'u1' : 'u2' })
          .all()
          .then((r: any) => r.length),
      ),
    ),
  ...
  console.log(
    `  server maxConnections=${maxConnections}: result=${JSON.stringify(outcome)} in ${(performance.now() - t).toFixed(0)}ms, server stats=${JSON.stringify(s.server.getStats())}`,
  )
  await db.close()
  await s.stop()
}

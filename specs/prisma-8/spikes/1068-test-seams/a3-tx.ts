...
const race = <T>(p: Promise<T>, ms: number) =>
  Promise.race([p, new Promise<string>((r) => setTimeout(() => r(`TIMEOUT ${ms}ms`), ms))])
const causeChain = (e: any) => {
  const out: string[] = []
  let c = e
  while (c) {
    out.push(`${c.constructor?.name}: ${String(c.message).slice(0, 100)}`)
    c = c.cause
  }
  return out.join(' <- ')
}
...
const variants: Variant[] = [
  ...
  {
    label: 'url, verifyMarker:false, no warm-up',
    maxConnections: 10,
    verifyMarker: false,
    warmup: false,
    n: 5,
  ...
  {
    label: 'pg Pool max=1, verifyMarker:false',
    maxConnections: 1,
    poolMax: 1,
    verifyMarker: false,
    warmup: false,
  ...
  {
    label: 'pg Pool max=1, default verifyMarker, no warm-up',
    maxConnections: 1,
    poolMax: 1,
    warmup: false,
    n: 1,
  ...
  {
    label: 'pg Pool max=1, default verifyMarker, WARM-UP',
    maxConnections: 1,
    poolMax: 1,
    warmup: true,
    n: 5,
  ...
  {
    label: 'pg Pool max=2, default verifyMarker, no warm-up',
    maxConnections: 2,
    poolMax: 2,
    warmup: false,
    n: 5,
...
for (const v of variants) {
  ...
  console.log(
    `${v.label.padEnd(52)} n=${v.n} -> ${JSON.stringify(outcome)} in ${(performance.now() - t).toFixed(0)}ms  server=${JSON.stringify(s.server.getStats())}`,
  )
  // teardown even if hung: end pool with force
  try {
    await race(db.close(), 2000)
  } catch {}
  try {
    await race(pool?.end() ?? Promise.resolve(), 2000)
  } catch {}
  try {
    await race(s.stop(), 3000)
  } catch {}
}
process.exit(0)

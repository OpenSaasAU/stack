...
type Call =
  | { kind: 'query'; text: string; values?: unknown[]; name?: string; hasTypes: boolean }
  | { kind: string }
const calls: Call[] = []
...
const fakeClient = {
  ...
  on(ev: string, _fn: unknown) {
    calls.push({ kind: `on(${ev})` })
    return this
  },
  async connect() {
  ...
  async query(cfg: any, values?: unknown[]) {
    if (typeof cfg === 'string') calls.push({ kind: 'query', text: cfg, values, hasTypes: false })
      ...
      calls.push({
        ...
        values: cfg.values,
        name: cfg.name,
        hasTypes: 'types' in cfg,
      })
    const text = typeof cfg === 'string' ? cfg : cfg.text
    if (/marker/i.test(text)) {
      markerSql.push(text)
...
dump('where().all()  [first query: includes marker read]')
console.log('   marker SQL seen:', JSON.stringify(markerSql))
...
for (const v of [
  ...
  try {
    ...
    console.log(
      `   row.published=${JSON.stringify(v.published)} -> ${JSON.stringify(r.published)} (${typeof r.published})`,
    )
  } catch (e: any) {
    console.log(
      `   row.published=${JSON.stringify(v.published)} -> THROWS ${String(e.message).slice(0, 100)}`,
    )
  }
}
calls.length = 0
...
canned = {
  rows: [{ id: 'p1', title: 'hi', published: true, authorId: 'u1', extra: 1 }],
  rowCount: 1,
}
console.log('   extra col ->', JSON.stringify(await db.orm.public.Post.first()))
canned = { rows: [{ id: 'p1' }], rowCount: 1 }
try {
  console.log('   missing cols ->', JSON.stringify(await db.orm.public.Post.first()))
} catch (e: any) {
  console.log('   missing cols -> THROWS', String(e.message).slice(0, 160))
}
calls.length = 0
  ...
  rows: [
    {
      id: 'u1',
      name: 'alice',
      posts: [{ id: 'p1', title: 'hi', published: true, authorId: 'u1' }],
    },
  ],
  rowCount: 1,
}
try {
  const r = await db.orm.public.User.include('posts').all()
  dump('include')
  console.log('   ->', JSON.stringify(r))
} catch (e: any) {
  dump('include')
  console.log('   THROWS', String(e.message).slice(0, 200))
}
...
const probe = new Proxy({ escapeIdentifier: () => '', escapeLiteral: () => '' } as any, {
  ...
  has(t, k) {
    touched.add('has:' + String(k))
    return k in t || ['on', 'query', 'connect', 'end'].includes(String(k))
  },
})
const db2: any = postgres({ contract, pg: probe })
await db2.orm.public.Post.all()
await db2.orm.public.Post.create({ id: 'x', title: 'x', published: false, authorId: 'u1' }).catch(
  () => {},
)
await db2.close()
...
dump('verifyMarker:false where().all()')
console.log('   marker SQL seen:', JSON.stringify(markerSql))
await db3.close()

...
const causeChain = (e: any) => {
  const out: string[] = []
  let c = e
  while (c) {
    out.push(`${c.constructor?.name}: ${String(c.message).slice(0, 140)}`)
    c = c.cause
  }
  return out.join('  <-  ')
}
...
console.log('== unix socket binding variants (server maxConnections=1) ==')
{
  ...
  const seedPool = new pg.Pool({ host: dir, database: 'postgres', user: 'postgres', max: 1 })
  await seed(seedPool)
  await seedPool.end()
  const variants: Array<[string, () => any]> = [
    ['url ?host=<dir>', () => postgres({ contract, url: s.url })],
    [
      'pg: Pool({host: dir, max: 1})',
      () => {
    ...
    [
      'pg: Pool({host: dir}) default max=10',
      () => {
        const pool = new pg.Pool({ host: dir, database: 'postgres', user: 'postgres' })
        const c: any = postgres({ contract, pg: pool })
        c.__pool = pool
        return c
      },
    ...
    [
      ...
      () =>
        postgres({
          contract,
          pg: new pg.Client({ host: dir, database: 'postgres', user: 'postgres' }),
        }),
    ],
  ]
  for (const [label, mk] of variants) {
    const db: any = mk()
    try {
      const r = await db.orm.public.Post.all()
      console.log(`  ${label.padEnd(40)} OK ${r.length} rows`)
    } catch (e: any) {
      console.log(`  ${label.padEnd(40)} FAIL: ${causeChain(e)}`)
    }
    try {
      await db.close()
      await db.__pool?.end()
...
)
{
  ...
  console.log(
    `  after first .all(): pool.totalCount=${pool.totalCount} server active=${s.server.getStats().activeConnections}`,
  )
  await db.orm.public.Post.all()
...
for (const [maxConnections, poolMax, label] of [
  [1, undefined, 'url (own pool, pg default max=10)'],
  [1, 1, 'pg: Pool max=1'],
  [5, undefined, 'url, server maxConnections=5'],
  [10, undefined, 'url, server maxConnections=10'],
] as const) {
  ...
  try {
    ...
    outcome = 'ERROR ' + causeChain(e)
  }
  console.log(
    `  server maxConnections=${maxConnections} ${label.padEnd(36)} -> ${JSON.stringify(outcome)} in ${(performance.now() - t).toFixed(0)}ms stats=${JSON.stringify(s.server.getStats())}`,
  )
  try {
    await db.close()
    await pool?.end()

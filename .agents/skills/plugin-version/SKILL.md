---
name: plugin-version
description: Bump plugin versions in the OpenSaaS Stack monorepo. Invoke whenever any file under Codex-plugins/* or .Codex-plugin/ is modified — skills, commands, agents, or marketplace config. Plugin versions must be bumped before committing so users who install the plugin get the updated version. Note: this is separate from pr-changeset, which handles npm packages — plugin versions are managed directly in plugin.json and marketplace.json.
allowed-tools: Read, Edit, Bash, Grep, Glob
---

# Plugin Version Skill

This skill bumps version numbers for Codex plugins and the marketplace manifest. Plugin versions live directly in JSON files (not changesets) — bumping them is what signals to the marketplace that something changed and triggers users to get the update.

## Files This Skill Manages

| Changed directory                    | Files to update                                                                                                                                             |
| ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Codex-plugins/opensaas-stack/`     | `Codex-plugins/opensaas-stack/.Codex-plugin/plugin.json` (version) AND `.Codex-plugin/marketplace.json` (plugins[name="opensaas-stack"].version)         |
| `Codex-plugins/opensaas-migration/` | `Codex-plugins/opensaas-migration/.Codex-plugin/plugin.json` (version) AND `.Codex-plugin/marketplace.json` (plugins[name="opensaas-migration"].version) |
| `.Codex-plugin/` root files only    | `.Codex-plugin/marketplace.json` (metadata.version only)                                                                                                   |
...
- `Codex-plugins/opensaas-stack/` → opensaas-stack plugin changed
- `Codex-plugins/opensaas-migration/` → opensaas-migration plugin changed
- `.Codex-plugin/` → marketplace root changed (check if any file other than version fields changed)
...
```
Read: Codex-plugins/<plugin-name>/.Codex-plugin/plugin.json
```
...
```
Read: .Codex-plugin/marketplace.json
```
...
Example in `Codex-plugins/opensaas-stack/.Codex-plugin/plugin.json`:
...
The `plugins` array in `.Codex-plugin/marketplace.json` has one object per plugin. Find the object where `"name"` matches the plugin name and edit its `"version"` field.
...
Only bump `metadata.version` if files in the `.Codex-plugin/` root directory were directly changed (e.g., `MARKETPLACE.md`, new fields added to `marketplace.json` structure, owner/metadata fields edited). Do NOT bump it just because plugin versions changed.
...
Changed files: `Codex-plugins/opensaas-stack/skills/some-skill/SKILL.md`

Current version: `"0.2.0"` → New version: `"0.2.1"`

Edit `Codex-plugins/opensaas-stack/.Codex-plugin/plugin.json`:

- `"version": "0.2.0"` → `"version": "0.2.1"`

Edit `.Codex-plugin/marketplace.json` (in the `opensaas-stack` plugins entry):
...
Changed files: `Codex-plugins/opensaas-migration/skills/new-skill/SKILL.md` (new file)

Current version: `"0.2.0"` → New version: `"0.3.0"`

Edit `Codex-plugins/opensaas-migration/.Codex-plugin/plugin.json`:

- `"version": "0.2.0"` → `"version": "0.3.0"`

Edit `.Codex-plugin/marketplace.json` (in the `opensaas-migration` plugins entry):

- `"version": "0.2.0"` → `"version": "0.3.0"`

### Example 3: Changes to both plugins

Changed files include both `Codex-plugins/opensaas-stack/...` and `Codex-plugins/opensaas-migration/...`
...
Edit `Codex-plugins/opensaas-stack/.Codex-plugin/plugin.json` with its new version.
Edit `Codex-plugins/opensaas-migration/.Codex-plugin/plugin.json` with its new version.
Edit `.Codex-plugin/marketplace.json` twice — once for each plugin's entry.

### Example 4: Marketplace-only change

Changed files: `.Codex-plugin/marketplace.json` (added a new top-level field) or `.Codex-plugin/MARKETPLACE.md`

The marketplace structure itself changed, so bump `metadata.version`.

Current: `"version": "1.1.0"` → New: `"version": "1.1.1"` (patch)

Edit `.Codex-plugin/marketplace.json` `metadata.version` field only.
...
A: These are independent. `pr-changeset` handles npm packages in `packages/*`. This skill handles Codex plugins in `Codex-plugins/*`. If a PR changes both, use both skills.

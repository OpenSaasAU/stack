...
const MINIMAL_CONFIG: OpenSaasConfig = { db: { provider: 'postgresql' }, lists: {} }

describe('richText() contract field', () => {
  it('is a non-nullable jsonb column when required', () => {
    const descriptor = richText({ validation: { isRequired: true } }).getContractField!(
      'content',
      'Article',
      MINIMAL_CONFIG,
    )
    if (descriptor.kind !== 'column') throw new Error('expected a column descriptor')
    expect(descriptor.nullable).toBe(false)
  })

  it('is a nullable jsonb column by default', () => {
    const descriptor = richText().getContractField!('excerpt', 'Article', MINIMAL_CONFIG)
    if (descriptor.kind !== 'column') throw new Error('expected a column descriptor')

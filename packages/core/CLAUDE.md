...
- **`@opensaas/stack-core`** (root) — the everyday consumer surface: `config`, `list`, `getContext`, the naming helpers (`getUrlKey`, `getListKeyFromUrl`), `ValidationError`, `InvalidSessionError` (thrown by `getContext`/`withSession`/`createTestContext` when a session holds `undefined` for one of its own keys), the stack-owned database errors (`DatabaseError`, `SerializationFailure`, `UniqueConstraintViolation`, `isSerializationFailure`, `isUniqueConstraintViolation`), and the config/access types you annotate with (`OpenSaasConfig`, `ListConfig`, `FieldConfig`, `AccessControl`, `FieldAccess`, `Session`, `AccessContext`, `OperationAccess`, `Where`, `OrderBy`).
...
- **Typed as a plain `string[]`, not narrowed to this list's own relation keys** — unlike `BaseFieldConfig`'s own `TKey` (issue #1306, threaded through `hooks` so a field's `resolveOutput`/`resolveInput` sees its real value type rather than `unknown`). A `needs` entry names a _different_ field on the same list, so narrowing its type would need that field's own key threaded too, for no benefit `pnpm generate` (`validateNeedsDeclarations`) doesn't already provide by rejecting a `needs` entry that isn't a stored column or an immediate relationship field on the same list — and a `needs` on a field with no `resolveOutput` hook — naming the field and the bad entry.

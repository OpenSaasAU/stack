...
- **Typed as a plain `string[]`, not narrowed to this list's own relation keys** — unlike `BaseFieldConfig`'s own `TKey` (issue #1306, threaded through `hooks` so a field's `resolveOutput`/`resolveInput` sees its real value type rather than `unknown`). A `needs` entry names a *different* field on the same list, so narrowing its type would need that field's own key threaded too, for no benefit `pnpm generate` (`validateNeedsDeclarations`) doesn't already provide by rejecting a `needs` entry that isn't a stored column or an immediate relationship field on the same list — and a `needs` on a field with no `resolveOutput` hook — naming the field and the bad entry.

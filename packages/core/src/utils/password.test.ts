...
import { hashPassword, comparePassword, isHashedPassword, HashedPassword } from './password.js'

// TypeScript rejects `==`/`===` between `HashedPassword` and `string` as a
// comparison with no overlap (TS2367); the `valueOf` coercion those operators
// perform is exactly what these tests assert, so the operands widen here.
...
describe('Password Utilities', () => {
  ...
  describe('HashedPassword class', () => {
    ...
    it('should keep returning the raw hash from string-coercion surfaces', async () => {
      ...
      expect(looseEquals(wrapped, hash)).toBe(true)
    })
    ...
    it('should work in comparisons and object operations', async () => {
      ...
      expect(looseEquals(wrapped, hash)).toBe(true)
      expect(strictEquals(wrapped, hash)).toBe(false) // Not strictly equal (different types)

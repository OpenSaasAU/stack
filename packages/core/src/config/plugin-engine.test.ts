...
import { text, integer } from '../fields/index.js'
...
describe('Plugin Engine', () => {
    ...
    test('handles complex dependency chains', async () => {
      ...
      const plugins: Plugin[] = [
        ...
        },
        {
          ...
          dependencies: ['auth'],
          init: async () => {
            executionOrder.push('notifications')
          },
        },
        {
          name: 'admin',
  ...
  describe('field type registration', () => {
    test('registers custom field types', async () => {
      const customFieldBuilder = (): BaseFieldConfig<TypeInfo> => ({
        type: 'customField',
  ...
  describe('config preservation', () => {
    test('preserves original config properties', async () => {
      ...
      const config: OpenSaasConfig = {
        db: { provider: 'postgresql' },
        storage: {
          uploads: { type: 's3', bucket: 'my-bucket' },
        },
        lists: {
          Post: {
            fields: { title: text() },

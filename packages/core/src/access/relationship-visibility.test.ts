...
import { filterReadableFields, getRelatedListConfig } from './index.js'
import { relationship, select, text } from '../fields/index.js'
import type { OpenSaasConfig, AccessContext } from '../index.js'
import type { FieldsWithTypeInfo, TypeInfo } from '../config/types.js'
        ...
        const config: OpenSaasConfig = {
          ...
          lists: {
            ...
            Post: {
              fields: {
                title: text(),
                internalNotes: text({ access: { read: () => false } }),
              },
              access: {
                operation: {
                  query: () => true,
    ...
    describe('no depth cap on an acyclic chain (issue #830)', () => {
      ...
      it('applies field-level read access and resolveOutput at depth 6+', async () => {
        ...
        for (let i = 0; i < chainLength; i++) {
          ...
          const fields: FieldsWithTypeInfo<TypeInfo> = { name: text() }
          if (isLast) {
            fields.secret = text({ access: { read: () => false } })
            fields.label = text({
              hooks: { resolveOutput: ({ value }) => `resolved:${value}` },
            })
          }
          if (i < chainLength - 1) {
            fields.next = relationship({ ref: `L${i + 1}.prev` })
          }
          if (i > 0) {
            fields.prev = relationship({ ref: `L${i - 1}.next` })
          }
          lists[`L${i}`] = {
            fields,

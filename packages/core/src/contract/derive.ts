 ...
 *   field's own create validator accepts an omission. A `validation.isRequired`
 *   text column therefore keeps no default — carrying one would drop the column
 *   from the required half of the generated create input while the validator
 *   still refused the omission — which leaves the flag inert for Keystone's
 *   commonest text column.
 */

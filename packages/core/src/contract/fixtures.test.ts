...
function table(contract: PrismaContract, name: string) {
  const found = contract.storage.namespaces.public.entries.table?.[name]
...
describe('deriveContract + buildPrismaContract — the fixtures yield a valid contract', () => {
  test('blog: ids by strategy, the singleton id, temporal timestamps, the synthetic back-relation', () => {
    ...
    const contract = buildPrismaContract(data)
    ...
    expect(table(contract, 'User').columns.createdAt).toMatchObject({
    ...
    expect(table(contract, 'Post').uniques).toEqual([{ columns: ['slug'] }])
    expect(table(contract, 'User').uniques).toEqual([{ columns: ['email'] }])
  ...
  test('auth: mapped tables, cascading non-null foreign keys, a named unique entry', () => {
    ...
    expect(table(contract, 'session').foreignKeys[0]).toMatchObject({
      source: { tableName: 'session', columns: ['userId'] },
      target: { tableName: 'user', columns: ['id'] },
      onDelete: 'cascade',
    })
    expect(table(contract, 'session').indexes.map((index) => index.columns)).toEqual([['userId']])
    expect(table(contract, 'session').uniques).toEqual([{ columns: ['token'] }])
...
describe('native types — every honoured db.nativeType lowers to its own column', () => {
  ...
  const contract = buildPrismaContract(data)
  const columns = table(contract, 'Reading').columns

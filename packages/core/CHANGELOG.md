...
- [#1420](https://github.com/OpenSaasAU/stack/pull/1420) [`7ebd6ee`](https://github.com/OpenSaasAU/stack/commit/7ebd6ee5f78f5773b566dc6cac66d73b640c3c03) Thanks [@borisno2](https://github.com/borisno2)! - Every write through `context.db` opens a real Prisma 8 transaction again
  ...
  trade for [#1262](https://github.com/OpenSaasAU/stack/issues/1262): a context that can silently downgrade every write's
...
- [#1556](https://github.com/OpenSaasAU/stack/pull/1556) [`477a75b`](https://github.com/OpenSaasAU/stack/commit/477a75bbfd2ee6e95ed78941b837cad7eea6a5f1) Thanks [@borisno2](https://github.com/borisno2)! - Fix a read-restricted `embedding()` field re-embedding on every write forever: the regeneration skip read the stored hash off the write's own Field-Visibility-filtered projection instead of the row, so a session denied read on the field always saw `undefined` and paid for a fresh provider call on every subsequent write ([#1282](https://github.com/OpenSaasAU/stack/issues/1282)). The regeneration check now costs one extra id-scoped read per write on a list with an `autoGenerate` embedding field, whether or not the field is read-restricted — a fixed, small price for correctness the in-memory comparison could no longer pay for.
...
- [#1495](https://github.com/OpenSaasAU/stack/pull/1495) [`cfb397a`](https://github.com/OpenSaasAU/stack/commit/cfb397ad72f1b27c854b0a765ebb249fbc7dab3e) Thanks [@borisno2](https://github.com/borisno2)! - `getContext`, `withSession` and `createTestContext` now refuse a session that holds `undefined` for one of its own keys

  `session ?? null` only treats `null`/`undefined` themselves as anonymous — a non-null object is signed in even when every property on it is `undefined`. `getContext({ userId })` from an unguarded `userId?: string` builds exactly that object, and an access rule written as `session === null ? ... : true` took its signed-in branch for an anonymous caller ([#1397](https://github.com/OpenSaasAU/stack/issues/1397), the defect behind [#1387](https://github.com/OpenSaasAU/stack/issues/1387)).
...
- [#1548](https://github.com/OpenSaasAU/stack/pull/1548) [`5334468`](https://github.com/OpenSaasAU/stack/commit/5334468181b2716897c0fce29965fc590b1dc239) Thanks [@borisno2](https://github.com/borisno2)! - Fix the generated context's client singleton so "once per process" holds in every environment, not only outside production: two bundler-duplicated copies of `.opensaas/context.ts` now share one client through a process-wide registry instead of each opening its own pool (ADR-0070).

- [#1672](https://github.com/OpenSaasAU/stack/pull/1672) [`00c3760`](https://github.com/OpenSaasAU/stack/commit/00c3760449f890f5d365798f5b3b3e1367333762) Thanks [@borisno2](https://github.com/borisno2)! - Fix `context.db.<List>.create()` throwing unconditionally for a field carrying both a `defaultValue` and an unconditional field-level `create` deny (`access: { create: () => false }`) — the value `applyCreateDefaults` fills in for an omitted field was being rejected by the deny meant for the caller, not the field's own default. Adds an opt-in `FieldAccess.allowCreateDefault` flag: when set, a field's own declared default is exempted from its `create` deny for a session that omitted the field, while an explicit attempt to write the field is still refused. Off by default, so a session-dependent `create` rule (denies one session, allows another) still blocks the whole create for a denied session exactly as before.

- [#1576](https://github.com/OpenSaasAU/stack/pull/1576) [`17bb5d5`](https://github.com/OpenSaasAU/stack/commit/17bb5d5a3afa78007afdd80eaa5a09f27a408890) Thanks [@borisno2](https://github.com/borisno2)! - Fix a singleton's `get({ include })` silently including a relation whole when given a nested shape (e.g. `{ take: 5 }`) instead of a boolean. It is now refused with a `ValidationError` naming the offending key, and the type only admits `true`/`false` per relation.

- [#1542](https://github.com/OpenSaasAU/stack/pull/1542) [`b1f838b`](https://github.com/OpenSaasAU/stack/commit/b1f838b2df2aae4e6edf2598b28b6bec2b8bfba7) Thanks [@borisno2](https://github.com/borisno2)! - Fix the test harness's `truncate()` opening a second `pg.Client` beside its declared `max: 1` pool; it now runs through the harness's own client instead.
...
- [#1579](https://github.com/OpenSaasAU/stack/pull/1579) [`e5ea5b3`](https://github.com/OpenSaasAU/stack/commit/e5ea5b3bd803106ed00a892ddffcdc365f4cde1e) Thanks [@borisno2](https://github.com/borisno2)! - Fix a joined write's `afterTransaction` hook receiving the transaction-bound context instead of the base client (ADR-0028), which refused a compensating write issued after the enclosing transaction had settled — while preserving that write's own `sudo()`/`withSession()` elevation.
...
- [#1580](https://github.com/OpenSaasAU/stack/pull/1580) [`6b40b3a`](https://github.com/OpenSaasAU/stack/commit/6b40b3aaa6174530a990a8f258778993f9501c98) Thanks [@borisno2](https://github.com/borisno2)! - MCP `query`'s id boundary coercion (ADR-0048) now walks the whole `where` predicate — every `AND`/`OR`/`NOT` branch, a relation quantifier's nested predicate (against the related list's own id strategy), and a nested relation-entry `where` inside a `fields` projection — instead of only the top level. A malformed id anywhere in that tree now refuses the whole request, the same answer `update`/`delete` already give, rather than silently matching nothing (which would widen a result under `NOT`).

- [#1514](https://github.com/OpenSaasAU/stack/pull/1514) [`e6a7fb2`](https://github.com/OpenSaasAU/stack/commit/e6a7fb28b57c56743537804231ed40e9d36ca4df) Thanks [@borisno2](https://github.com/borisno2)! - `writePluginOwnedField` now refuses a virtual field by name, instead of falling through to a write the database rejects as an unknown column and the RAG plugin then misreports as retryable.

- [#1581](https://github.com/OpenSaasAU/stack/pull/1581) [`f097958`](https://github.com/OpenSaasAU/stack/commit/f097958cba46015072b7b118d1ca78471700571e) Thanks [@borisno2](https://github.com/borisno2)! - Named the `ctx.runtime` path a collection carries off both `unsafe.orm` and `AccessContext.ormHandle` in the Unsafe surface's `Known limits`, corrected prose that overclaimed `prepare()`/`runtime()` as unreachable rather than "not among the surface's own members", and added a test pinning the reachability and tripwire behavior of `ctx.runtime` and `runtime.connection()`.

- [#1544](https://github.com/OpenSaasAU/stack/pull/1544) [`548dd00`](https://github.com/OpenSaasAU/stack/commit/548dd0014551b07c3ff3fefccb2945bb025c6864) Thanks [@borisno2](https://github.com/borisno2)! - Delete the dead to-one access tree, its visibility resolver and the post-query null-forcing (`access-filter.ts`), now that the native include-based scoping fully replaced them. Internal cleanup only — no behavior change.
...
- [#1503](https://github.com/OpenSaasAU/stack/pull/1503) [`5c23069`](https://github.com/OpenSaasAU/stack/commit/5c23069861fda7ebfd77ac0726200800b597d60a) Thanks [@borisno2](https://github.com/borisno2)! - Fix a to-one relationship label filter emitting `contains` against `id` (and throwing a `ValidationError`) for a related list with no `name`/`title`/`ui.labelField`; the filter now declines instead.

- [#1546](https://github.com/OpenSaasAU/stack/pull/1546) [`721b7c0`](https://github.com/OpenSaasAU/stack/commit/721b7c0eeca000966c7a419e3946d34cef9b2bc1) Thanks [@borisno2](https://github.com/borisno2)! - Add a test guarding that a tripwire refusal mid-write rolls back the write's transaction and reaches the caller as `UnmarkedQueryError`, unwrapped.

- [#1539](https://github.com/OpenSaasAU/stack/pull/1539) [`cae8c1e`](https://github.com/OpenSaasAU/stack/commit/cae8c1e562bf6f780bd2d870948f407dbabbf4fe) Thanks [@borisno2](https://github.com/borisno2)! - Fix `resolvePrismaBinary`'s script detection to also match a `.mjs`/`.cjs` Prisma CLI entry point, not only `.js`. Test-only fixes: correct a `bundle-typecheck.test.ts` name that overstated its own coverage (`skipLibCheck` means it never checks `contract.d.ts` for errors), cover the `longIdentifierConfig` fixture in the contract structural-equivalence suite, and move a misplaced fixture docblock onto the export it describes.

- [#1513](https://github.com/OpenSaasAU/stack/pull/1513) [`bc2614f`](https://github.com/OpenSaasAU/stack/commit/bc2614f3ebd662737d22d5e47bf03f26a1aca5a6) Thanks [@borisno2](https://github.com/borisno2)! - `bindContextToTransaction` now spreads the source `AccessContext` and overrides only what a transaction rebind must change, instead of rebuilding it by listing every member by hand — a shape that silently drops whichever member the next one forgets to add to the list.

- [#1533](https://github.com/OpenSaasAU/stack/pull/1533) [`c3ce7b6`](https://github.com/OpenSaasAU/stack/commit/c3ce7b6f23240144ae424d47685c71c0e553b877) Thanks [@borisno2](https://github.com/borisno2)! - Refuse, at `pnpm generate` time, an explicit `defaultValue` a field's own create validation would reject (e.g. `text({ defaultValue: '', validation: { isRequired: true } })`). Previously the value was written onto the contract column unconditionally, so the column's default dropped the field from `CreateInput`'s required half while the create validator still rejected it — an omitted create type-checked and then threw `ValidationError` at runtime.

- [#1575](https://github.com/OpenSaasAU/stack/pull/1575) [`7ffacbf`](https://github.com/OpenSaasAU/stack/commit/7ffacbfe1cee35ae7d9922519e94a68c13d0b34b) Thanks [@dependabot](https://github.com/apps/dependabot)! - Fix Vitest 5 browser test shutdown running out of memory and preserve per-file coverage gates.

- [#1584](https://github.com/OpenSaasAU/stack/pull/1584) [`a2c8e59`](https://github.com/OpenSaasAU/stack/commit/a2c8e59e39d303c97dbdb51875261fff0169be8d) Thanks [@borisno2](https://github.com/borisno2)! - Fix a computed field (`{ kind: 'computed' }`, e.g. from a third-party field builder) that omits core's `virtual` flag: its `resolveOutput` now runs on read (it silently never ran before), and the generated `needs`/`NeedsItem` types are produced for it, matching a flagged `virtual()` field.

- [#1567](https://github.com/OpenSaasAU/stack/pull/1567) [`f06fbf4`](https://github.com/OpenSaasAU/stack/commit/f06fbf4055f51946d89c72985176d54f43af217d) Thanks [@borisno2](https://github.com/borisno2)! - The write pipeline's per-write ORM handle reconciliation now derives the list-to-namespace shape once per context, reused on every write's transaction, instead of re-walking `config.lists` on each write.

- [#1562](https://github.com/OpenSaasAU/stack/pull/1562) [`6d8c6c7`](https://github.com/OpenSaasAU/stack/commit/6d8c6c78d590f0dc804578fd3f8f2b32b02677ce) Thanks [@borisno2](https://github.com/borisno2)! - Fix `packages/core/CLAUDE.md` documenting the plugin runtime's `sudo()` helper as returning `AccessContext` — it returns `StackContext`.

- [#1569](https://github.com/OpenSaasAU/stack/pull/1569) [`5cfa783`](https://github.com/OpenSaasAU/stack/commit/5cfa7832f48f905391c5a76b979a9ca93024d207) Thanks [@borisno2](https://github.com/borisno2)! - Remove `ListUniqueWhere`, `ListFilterArgs`, `ListWhere`, `ListOrderBy` and `ColumnFilter` — dead exports left over from a `findMany`/`findUnique`-style args surface `update`/`delete` no longer use (they take `ListIdentityWhere`, i.e. `{ id }`, and `findUnique` itself no longer exists).

- [#1521](https://github.com/OpenSaasAU/stack/pull/1521) [`e11271b`](https://github.com/OpenSaasAU/stack/commit/e11271b523cf92f9d60236ea02715b1fa87c6028) Thanks [@borisno2](https://github.com/borisno2)! - Guard `typed-read-surface.test.ts`'s pgvector-dependent test so the Database escape skips it, rather than failing the whole file, on a server without the extension.
...
- [#1538](https://github.com/OpenSaasAU/stack/pull/1538) [`22f03bc`](https://github.com/OpenSaasAU/stack/commit/22f03bca2f190a9c1d854e22efe5beb3807da264) Thanks [@borisno2](https://github.com/borisno2)! - Fix three Dev database robustness gaps: `resolveDatabaseUrl()` now treats a state file whose pid is live but not this sidecar (a reboot or pid wrap) as stale; `startDevDatabase` refuses a second call against a `dataDir` already in use instead of racing it; and a relative `dataDir` now resolves against the same root as the state file.

- [#1487](https://github.com/OpenSaasAU/stack/pull/1487) [`12bd183`](https://github.com/OpenSaasAU/stack/commit/12bd183112000abe7d34487ee0f8c874b5623507) Thanks [@borisno2](https://github.com/borisno2)! - Fix a flaky test: `lower.test.ts`'s vector-lowering retry case no longer cold-imports the real ORM module inside the timed test body.

- [#1547](https://github.com/OpenSaasAU/stack/pull/1547) [`53f2d20`](https://github.com/OpenSaasAU/stack/commit/53f2d202d5a58bcdb4d016aef929ae68a5151ed4) Thanks [@borisno2](https://github.com/borisno2)! - Fix the test harness's escape to read `DIRECT_DATABASE_URL`/`DATABASE_URL` in the same order `resolveDatabaseUrl()` does, so a `DIRECT_DATABASE_URL`-only environment runs the suite against that server instead of silently falling back to PGlite.

- [#1502](https://github.com/OpenSaasAU/stack/pull/1502) [`d40c4bb`](https://github.com/OpenSaasAU/stack/commit/d40c4bb1700bc4f52203d4082cc13fb9efaa098a) Thanks [@borisno2](https://github.com/borisno2)! - Remove the dead `FindManyArgs`, `FindUniqueArgs` and `CountArgs` exports left behind by a prior delegate removal, and advertise/validate MCP `connect.id` at the related list's own id type instead of assuming every list is string-keyed.
...
- [#1609](https://github.com/OpenSaasAU/stack/pull/1609) [`04a00f4`](https://github.com/OpenSaasAU/stack/commit/04a00f4ae87611b879829b9d20fbfe9cdb656da2) Thanks [@borisno2](https://github.com/borisno2)! - Prevent production database discovery and tracing from following Dev state into the project root; pass an explicit state path when the dev loop serves a production build.

- [#1490](https://github.com/OpenSaasAU/stack/pull/1490) [`3d0d4db`](https://github.com/OpenSaasAU/stack/commit/3d0d4db7923a75cf77f665ef8f915edcf99a408d) Thanks [@borisno2](https://github.com/borisno2)! - Fix a flaky substring-over-serialised-JSON leak assertion in `select.test.ts` (same shape as the `include.test.ts` flake fixed in [#1479](https://github.com/OpenSaasAU/stack/issues/1479)), using a structural exact-value check instead.

- [#1586](https://github.com/OpenSaasAU/stack/pull/1586) [`80b3e20`](https://github.com/OpenSaasAU/stack/commit/80b3e204d20f60aded2d6d10cd9a04b8da5119da) Thanks [@borisno2](https://github.com/borisno2)! - Make an empty-value filter token (`role:` or a bare `""`) an explicit, tested no-op in `buildFilterWhere` instead of a silent drop that relied on JS falsy-string truthiness.
...
- [#1510](https://github.com/OpenSaasAU/stack/pull/1510) [`6eab4e2`](https://github.com/OpenSaasAU/stack/commit/6eab4e28e2a42168db2fb8bb99c94c1f22d527cd) Thanks [@borisno2](https://github.com/borisno2)! - Fix a stale item form's to-many deselect silently reverting a concurrent re-point of the related row. `removeRelated`'s disconnect now locks the row and compares its current back-reference against the form's baseline, refusing the unlink (with an error the form surfaces) instead of nulling a link someone else just wrote.
...
- [#1504](https://github.com/OpenSaasAU/stack/pull/1504) [`d9b4b37`](https://github.com/OpenSaasAU/stack/commit/d9b4b3740df3573d090b140eb32daaec5c319485) Thanks [@borisno2](https://github.com/borisno2)! - Fix a filter token that can't be compiled or degraded to free text (e.g. an unsupported operator on a list with no free-text field) silently widening the result set instead of matching no rows.
...
- [#1498](https://github.com/OpenSaasAU/stack/pull/1498) [`d049f38`](https://github.com/OpenSaasAU/stack/commit/d049f3804cc2974b98c854ff4ecfd1660e6d5a75) Thanks [@borisno2](https://github.com/borisno2)! - Bypassing the secured read funnel (`visibleRows`) is now a compile error, and a related row deleted between the main read and its companion foreign-key existence check fails closed to `null` — pinned by a regression test rather than left to review.
...
- [#1506](https://github.com/OpenSaasAU/stack/pull/1506) [`aa9296c`](https://github.com/OpenSaasAU/stack/commit/aa9296cd3af11e3e0c1b13969d986915e7673c9a) Thanks [@borisno2](https://github.com/borisno2)! - Fix MCP `create`/`update` tools disclosing which fields a session may not write: a field withheld from the `data` schema now refuses with the byte-identical message a nonexistent field gets, instead of a distinct "field-level access denied" message.
...
- [#1519](https://github.com/OpenSaasAU/stack/pull/1519) [`52b79a7`](https://github.com/OpenSaasAU/stack/commit/52b79a7519eadce9a2dc812e63a32df022f3c578) Thanks [@borisno2](https://github.com/borisno2)! - Fix `context.plugins` and the plugin data registry being plain objects keyed by an author-supplied plugin name — a plugin named `__proto__` corrupted the object's prototype, and a name like `constructor` shadowed an inherited member. Registries are now built null-prototype, and a reserved plugin name is now refused at config time with a clear error.

- [#1507](https://github.com/OpenSaasAU/stack/pull/1507) [`781b819`](https://github.com/OpenSaasAU/stack/commit/781b8196064835637b93e34ec31e6a3dc959ccae) Thanks [@borisno2](https://github.com/borisno2)! - MCP `tools/list` no longer advertises a `create` tool a session can never use: a row-independent `access.operation.create` denial now drops the tool, and a `db: { isNullable: false }` required field is now classified the same as `validation.isRequired`.
...
- [#1566](https://github.com/OpenSaasAU/stack/pull/1566) [`6975d3e`](https://github.com/OpenSaasAU/stack/commit/6975d3e76cb0280d9960461698e079276b56f8c0) Thanks [@borisno2](https://github.com/borisno2)! - Delete the dead relationship-count module; `isToManyRelationshipField` moves to `fields/index.ts` with no change to its root export.

- [#1482](https://github.com/OpenSaasAU/stack/pull/1482) [`eedb0f7`](https://github.com/OpenSaasAU/stack/commit/eedb0f7b4b856fea9927c2a06f827d38fc28f75c) Thanks [@borisno2](https://github.com/borisno2)! - Render `decimal()` fields in the admin UI
...
- [#1537](https://github.com/OpenSaasAU/stack/pull/1537) [`a4227e9`](https://github.com/OpenSaasAU/stack/commit/a4227e9dd8097a38ec80a430b725dc5f0838f8c2) Thanks [@borisno2](https://github.com/borisno2)! - Fix `CreateInput`/`UpdateInput` never offering a multi-column field's assembled logical key (`db: { columns: 'keystone' }`, and `@opensaas/stack-rag`'s `embedding()`), and fix its raw physical columns leaking onto the row, create and update types instead of being hidden behind it. The generated bundle now carries one more remainder fact — which physical columns a multi-column field owns — so `context.db.Article.create({ data: { hero: uploadedFile } })` type-checks and `article.hero_url` no longer does.
...
- [#1575](https://github.com/OpenSaasAU/stack/pull/1575) [`7ffacbf`](https://github.com/OpenSaasAU/stack/commit/7ffacbfe1cee35ae7d9922519e94a68c13d0b34b) Thanks [@dependabot](https://github.com/apps/dependabot)! - Avoid CI timeouts in password behavior tests by using low-cost bcrypt fixtures while retaining coverage of the production default cost.

- [#1560](https://github.com/OpenSaasAU/stack/pull/1560) [`b0317b4`](https://github.com/OpenSaasAU/stack/commit/b0317b475ecd2cff6f11fe42ef964c80696c60ec) Thanks [@borisno2](https://github.com/borisno2)! - Fix `UndefinedAccessFilterError`'s suggested sample code, which only used the first path segment and so rendered an invalid shape for a nested or composite condition (an `AND`/`OR` branch, an operator, or a relation quantifier).

- [#1518](https://github.com/OpenSaasAU/stack/pull/1518) [`bed0cc3`](https://github.com/OpenSaasAU/stack/commit/bed0cc3197cd4b1ab5aebf40eda6289ae8a955fd) Thanks [@borisno2](https://github.com/borisno2)! - Fix a stale test expectation in `filter.test.ts` that still asserted the pre-[#1504](https://github.com/OpenSaasAU/stack/issues/1504) `undefined` where for an undegradable filter token, causing the `test` job to fail on unrelated PRs.

- [#1587](https://github.com/OpenSaasAU/stack/pull/1587) [`0a3cba5`](https://github.com/OpenSaasAU/stack/commit/0a3cba5197d98ff780036a0805d660af84dfbc50) Thanks [@borisno2](https://github.com/borisno2)! - Fix a computed field (`{ kind: 'computed' }` with no `virtual` flag) whose value was written on `create`/`update` instead of skipped — the same bug [#1531](https://github.com/OpenSaasAU/stack/issues/1531) fixed on the read path, missed on the write path (`filterWritableFields`).
...
- [#1563](https://github.com/OpenSaasAU/stack/pull/1563) [`07999bb`](https://github.com/OpenSaasAU/stack/commit/07999bb4c6e7748f8b3434ce82db780dbf6a8e9f) Thanks [@borisno2](https://github.com/borisno2)! - Fix three MCP query-tool argument residuals: root `take`/`skip` are now type-checked like the nested selector, root `take: 0` is honoured as zero rows instead of falling back to 10, and the count-only projection path falls back to `0` instead of an unrecognised shape that could carry rows.

- [#1559](https://github.com/OpenSaasAU/stack/pull/1559) [`b84c28d`](https://github.com/OpenSaasAU/stack/commit/b84c28d2fd5088fdb56a7ba30e719e4f52b57d7d) Thanks [@borisno2](https://github.com/borisno2)! - Fix the row-lock terminal's name in error messages: it now reads `forUpdate().all()`/`forUpdate().first()`, matching the order that actually type-checks.

...
- [#1561](https://github.com/OpenSaasAU/stack/pull/1561) [`57ce2d1`](https://github.com/OpenSaasAU/stack/commit/57ce2d11880065ba8fec7930abe445a299c90357) Thanks [@borisno2](https://github.com/borisno2)! - `ChunkingConfig`'s `strategy` field used a type also named `ChunkingStrategy`
...
- [#1607](https://github.com/OpenSaasAU/stack/pull/1607) [`e1a2a20`](https://github.com/OpenSaasAU/stack/commit/e1a2a2080f8bf0fa9bd0bde82dbc92428ed5d0cb) Thanks [@borisno2](https://github.com/borisno2)! - Keep OpenAI provider unit tests deterministic and offline-safe
...
- [#1675](https://github.com/OpenSaasAU/stack/pull/1675) [`b24c83b`](https://github.com/OpenSaasAU/stack/commit/b24c83b76ae5e3d8d434fd97400aac5000ff59bd) Thanks [@borisno2](https://github.com/borisno2)! - Fix a security leak where a session denied read access to an `embedding()`'s `sourceField` could still read the embedding's vector, its `sourceHash`, and rank rows with `nearest()` (issue [#1627](https://github.com/OpenSaasAU/stack/issues/1627), ADR-0045 amended).
...
- [#1540](https://github.com/OpenSaasAU/stack/pull/1540) [`06c1725`](https://github.com/OpenSaasAU/stack/commit/06c172533c416350e3b6e1960eb606c15a829ea8) Thanks [@borisno2](https://github.com/borisno2)! - Fix `findSimilar`'s `excludeSelf` overwriting a caller's own `id` predicate in `where` instead of combining with it.
...
- [#1528](https://github.com/OpenSaasAU/stack/pull/1528) [`dd30746`](https://github.com/OpenSaasAU/stack/commit/dd30746f510deec405473354502640de67033a31) Thanks [@borisno2](https://github.com/borisno2)! - Remove `chunking` from `embedding()`, `searchable()` and `ragPlugin()`: it was accepted and silently ignored, since a field is one native vector column per row. Long documents now need a dedicated per-chunk list — documented in the RAG how-to guide.

- [#1527](https://github.com/OpenSaasAU/stack/pull/1527) [`ebdd6c6`](https://github.com/OpenSaasAU/stack/commit/ebdd6c6c8f57a868213f3447c460784b9509892d) Thanks [@borisno2](https://github.com/borisno2)! - `OllamaEmbeddingProvider` now refuses a missing or non-positive-integer `dimensions`, naming the model and the bad value, instead of silently accepting it — `dimensions` decides the generated column's width, so a bad value should fail loudly rather than propagate. The `registerEmbeddingProvider('custom', ...)` example in the RAG guidance is corrected to match: it now refuses a missing/invalid `dimensions` instead of silently defaulting to 768.
...
- [#1512](https://github.com/OpenSaasAU/stack/pull/1512) [`3e04870`](https://github.com/OpenSaasAU/stack/commit/3e04870f2ed36948f784325ea268bba4d82aef4d) Thanks [@borisno2](https://github.com/borisno2)! - Fix the generation hook's escalated-write lookup throwing past its guard on a missing plugin context, which surfaced as a failure off an already-committed write instead of being logged as a standing defect.
...
- [#1541](https://github.com/OpenSaasAU/stack/pull/1541) [`e589b2c`](https://github.com/OpenSaasAU/stack/commit/e589b2c445d81c82b8645cb95c2a67eb199ddda3) Thanks [@borisno2](https://github.com/borisno2)! - `OllamaEmbeddingProvider.embed()` now fails with a named error (model, declared dimensions, real dimensions) when the model's returned vector doesn't match the declared `dimensions`, instead of writing a mismatched column that only surfaces as an opaque failure at search time.
...
- [#1553](https://github.com/OpenSaasAU/stack/pull/1553) [`4aa951e`](https://github.com/OpenSaasAU/stack/commit/4aa951e634e74e15572416bf4e42dde35d078194) Thanks [@borisno2](https://github.com/borisno2)! - Fix the `semantic_search_<list>` MCP tool spreading every embedding column's vector and metadata into each result, burning context and leaking a column that may carry its own read restriction.

- [#1501](https://github.com/OpenSaasAU/stack/pull/1501) [`1cd26c6`](https://github.com/OpenSaasAU/stack/commit/1cd26c68f19fb17e498ab62fcdd97a1cb3925e88) Thanks [@borisno2](https://github.com/borisno2)! - No functional change. The doc-block checker's fragment classifications (issue
  [#1382](https://github.com/OpenSaasAU/stack/issues/1382)) moved off a `file:line`-keyed `scripts/doc-blocks/fragments.json` and
...
- [#1523](https://github.com/OpenSaasAU/stack/pull/1523) [`8dba574`](https://github.com/OpenSaasAU/stack/commit/8dba574a966858da2d6238f8f8e3e3f0c938a103) Thanks [@borisno2](https://github.com/borisno2)! - Fix flaky `RateLimiter` under-limit test: assert no throttling delay was scheduled instead of asserting wall-clock duration.

- [#1524](https://github.com/OpenSaasAU/stack/pull/1524) [`daaa764`](https://github.com/OpenSaasAU/stack/commit/daaa764747995ebfd6c15dd644213c7865832a1f) Thanks [@borisno2](https://github.com/borisno2)! - Fix `sourceHash` being written by two disagreeing hash implementations, which made the auto-generation skip never match on a row hashed by the other one and forced a redundant paid re-embed on every subsequent write.

- [#1565](https://github.com/OpenSaasAU/stack/pull/1565) [`3865df1`](https://github.com/OpenSaasAU/stack/commit/3865df15d89a3a99af0584f76d5904cd14936e7b) Thanks [@borisno2](https://github.com/borisno2)! - `semanticSearch()`/`findSimilar()` now refuse a list with no vector column at compile time, matching `nearest()`'s own refusal, instead of accepting any `fieldName` and throwing at runtime.

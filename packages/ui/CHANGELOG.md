...
- [#1493](https://github.com/OpenSaasAU/stack/pull/1493) [`8d9ed68`](https://github.com/OpenSaasAU/stack/commit/8d9ed688b4c01afdab72247ad0e0715f535a2e24) Thanks [@borisno2](https://github.com/borisno2)! - Fix the admin sidebar rendering the signed-in user as the literal string `"undefined"`.
...
- [#1484](https://github.com/OpenSaasAU/stack/pull/1484) [`c18578c`](https://github.com/OpenSaasAU/stack/commit/c18578c72745a3308e3e8c992f97755e10cb4227) Thanks [@borisno2](https://github.com/borisno2)! - Render `json()` and `richText()` fields in the admin list view
...
- [#1508](https://github.com/OpenSaasAU/stack/pull/1508) [`22d4cac`](https://github.com/OpenSaasAU/stack/commit/22d4cacf8da63c409eb4e56e6b3f72d371b4b154) Thanks [@borisno2](https://github.com/borisno2)! - Defer the to-many "Link existing" control's far-endpoint options fetch to its first open instead of reading up to 50 rows per section on every item-view render.

- [#1526](https://github.com/OpenSaasAU/stack/pull/1526) [`6199999`](https://github.com/OpenSaasAU/stack/commit/6199999ad9a21b0ff244c866de6391dce92786d3) Thanks [@borisno2](https://github.com/borisno2)! - Add a standing test pinning the full chain from a genuine PostgreSQL unique-constraint violation to item-form state — `withOrigin` classification, `normalizeDatabaseError`'s constraint-map resolution, `serverAction`'s `{ error, fieldErrors }` envelope, and `ItemFormClient`/`useItemForm` — so a per-field message reaches the right field and no raw driver text leaks into it ([#1334](https://github.com/OpenSaasAU/stack/issues/1334)). No runtime change.
...
- [#1520](https://github.com/OpenSaasAU/stack/pull/1520) [`6ce87ce`](https://github.com/OpenSaasAU/stack/commit/6ce87ce94753ffb62049462d60ad6692a8ec2e97) Thanks [@borisno2](https://github.com/borisno2)! - The admin item view's relationship-table row removal now defaults to `'delete'` for a section backed by an explicit junction list, instead of `'disconnect'`, so removing an edge no longer leaves an orphaned junction row pointing at only one endpoint. An ordinary to-many back-reference still defaults to `'disconnect'`, and an explicit `removeAction` continues to win over the default in both cases.
...
- [#1578](https://github.com/OpenSaasAU/stack/pull/1578) [`1486029`](https://github.com/OpenSaasAU/stack/commit/1486029dedfb1e23052a297f4de2ceab552eb9ad) Thanks [@borisno2](https://github.com/borisno2)! - Fix `build:css` on Windows: replace the POSIX-only `mkdir -p` with a cross-platform `node -e` call, so the package builds outside a POSIX shell.

- [#1502](https://github.com/OpenSaasAU/stack/pull/1502) [`d40c4bb`](https://github.com/OpenSaasAU/stack/commit/d40c4bb1700bc4f52203d4082cc13fb9efaa098a) Thanks [@borisno2](https://github.com/borisno2)! - Type `CellComponentProps.value` (and the newly-exported `CellValue`) instead of leaving it as `unknown`, so a relationship Cell's null handling is a compile-time guarantee rather than only a runtime one.

...
- [#1535](https://github.com/OpenSaasAU/stack/pull/1535) [`5dcd372`](https://github.com/OpenSaasAU/stack/commit/5dcd37278012fe1d3541bf42b6e3c71442582788) Thanks [@borisno2](https://github.com/borisno2)! - `opensaas generate` now refuses when `migrations/` carries a contract-space directory for a pack no longer in `db.extensions`, naming the directory instead of exiting 0 and leaving `db init`/`db update` to fail later.

- [#1492](https://github.com/OpenSaasAU/stack/pull/1492) [`e51513e`](https://github.com/OpenSaasAU/stack/commit/e51513e18a38334a5badc1583734d6a597773f39) Thanks [@borisno2](https://github.com/borisno2)! - `opensaas generate` now loads the project's `.env` before reading the config, the same as `opensaas dev`, so a config that reads `process.env` at module scope sees consistent values under both commands. `@opensaas/stack-rag`'s runtime no longer loads `.env` as a side effect of import — a script that needs it now loads it explicitly (e.g. `tsx --env-file-if-exists=.env`).
...
- [#1568](https://github.com/OpenSaasAU/stack/pull/1568) [`035af0f`](https://github.com/OpenSaasAU/stack/commit/035af0f40ca4a5aaebd9ab920988c3e2f06bed7e) Thanks [@borisno2](https://github.com/borisno2)! - Fix the dev-database concurrency test to kill an orphaned `db update` child on any failure path, tighten its overlap-window sample, and log the overlap count on a passing run.

- [#1536](https://github.com/OpenSaasAU/stack/pull/1536) [`35a4abc`](https://github.com/OpenSaasAU/stack/commit/35a4abc834b5a91d15856b941230fb8fd98632c7) Thanks [@borisno2](https://github.com/borisno2)! - Fix `opensaas dev` deleting a newly-declared extension pack's ref when a config edit both adds the pack and carries a destructive change together, which stranded the pack refless and made `db update --confirm` fail.
...
- [#1564](https://github.com/OpenSaasAU/stack/pull/1564) [`d84d4f0`](https://github.com/OpenSaasAU/stack/commit/d84d4f0bf75b193284e4e90f74247f9d60aa6e97) Thanks [@borisno2](https://github.com/borisno2)! - Narrow the staged-promotion window in which `.opensaas/tables.ts` could disagree with `prisma/contract.ts`, and document `prisma.config.ts` always landing last as the stated promotion-complete signal (ADR-0072).
...
- [#1529](https://github.com/OpenSaasAU/stack/pull/1529) [`028bde7`](https://github.com/OpenSaasAU/stack/commit/028bde74320e572006f79cd14bafbdf867ad1a58) Thanks [@borisno2](https://github.com/borisno2)! - Fix the generated `Remainder`'s `computed`/`output` placement for a field whose contract descriptor is `kind: 'computed'` but does not also set the `virtual` flag — it now lands in `computed` by descriptor kind, matching the self-containment gate.

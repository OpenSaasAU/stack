...
function pgColumnExpression(model: string, column: ContractColumn): Rendered {
  ...
  switch (type.type) {
    ...
    case 'varchar': {
      ...
      if (length === undefined) {
        throw new Error(
          `Model "${model}", column "${column.name}": a varchar column needs a length`,
        )
      }
...
export function renderContractModule(data: ContractData): string {
  ...
  lines.push('')
  lines.push(`import { ${builderImports.join(', ')} } from '@prisma/orm-postgres/contract-builder'`)
  lines.push("import type { ScalarFieldBuilder } from '@prisma/orm-postgres/contract-builder'")
  ...
  scaffold.push(
    packNames.length === 0 ? 'extensions: {}' : `extensions: { ${packNames.join(', ')} }`,
  )

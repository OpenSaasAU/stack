...
describe('opensaas dev', () => {
  ...
  test('a production-mode dev loop reconciles before running its app', async () => {
    const projectDir = createProject('production-loop', ROW_PROBE)

    const run = await runDevLoop(projectDir, { NODE_ENV: 'production' })
